...
r"""This script greatly shortens the PROMPT_COMMAND introduced by lunch.

The default command looks like

  echo -ne "\033]0;[<build>] <ldap>@<fullhostname>: <cwd>\007"
...
def main():
  ...
  if not match:
    print("No lunch to fix (PROMPT_COMMAND has unrecognized format)",
          file=sys.stderr)
    print(current_env)
    exit(0)
